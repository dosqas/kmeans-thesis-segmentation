...
def get_fast_distance_circuit(num_features):
...
def qiskit_quantum_kmeans(samples, centers, k):
    """
    Simulates Quantum K-Means!
    For huge parameter batches at 30 FPS, building thousands of Qiskit Dictionaries 
    in pure python locks the CPU. 
    Instead, we compute the EXACT mathematical prediction that the quantum physics 
    simulator outputs (Matrix Product State Overlap), using Numpy's instant C-engine!
    """
    n = len(samples)
    ...
    # -------------------------------------------------------------
    global_range = np.max(np.ptp(samples, axis=0)) + 1e-8
    scale_factor = (np.pi / 2.0) / global_range
    
    # Scale exactly like we did before
    s_scaled = samples * scale_factor
    c_scaled = centers * scale_factor
...
def main():
    ...
    while True:
        try:
            ...
            t0 = time.time()
            new_centers = qiskit_quantum_kmeans(samples, centers, k)
